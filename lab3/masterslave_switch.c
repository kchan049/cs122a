...
void LEDS_Tick(){
	...
	static char counttimmer = 0; //number of loops you want
	char master = 0x01 & ~getbit(PINB, 0);
...
void SEND_Tick() {
	...
	switch(send_state){
		case initsend:
			break;
		case sendsend:
			if (0x01 & ~getbit(PINB, 0)) //if master
...
void REC_Tick() {
	...
	switch(rec_state){
		...
		break;
		case recrec:
		if (!(0x01 & ~getbit(PINB, 0))) //if slave
